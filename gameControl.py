...
"""
                                                ...
                                                Driving simulation using openCV
...
while True:
    ...
    # Video frame to be mirrored to get inverted image
    ...
    # storing height and width in variables
    ...
    # divide the frame into separate halves so that we can have one half control the turning/steering
    ...
    if len(cnts_up) > 0:
        ...
        # find circle of minimum area enclosing a 2D point set
    ...
    if len(cnts_down) > 0:
        ...
        if radius2 > circle_radius:
            ...
            if (height // 2) < center_down[1] < ((3 * height) // 4) and (width // 4) < center_down[0] < (
                    (3 * width) // 4):
    ...
    cv2.putText(frame_copy, 'RIGHT', (300, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))

    # draw box for left (W)
    frame_copy = cv2.rectangle(frame_copy, (width // 4, (height // 2) + 5), (3 * width // 4, 3 * height // 4),
                               (255, 255, 255), 1)
    cv2.putText(frame_copy, 'ACCELERATE', (width // 4, (height // 2) + 33), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))

    # draw box for left (S)
    frame_copy = cv2.rectangle(frame_copy, (width // 4, ((3 * height) // 4) + 5), (3 * width // 4, height),
                               (255, 255, 255), 1)
    cv2.putText(frame_copy, 'BRAKE', ((3 * width // 4) - 100, (height // 2) + 108), cv2.FONT_HERSHEY_SIMPLEX, 1,
                (255, 255, 255))
